...
const ByClickEventModal = () => {
    const { selectedEvent } = useSelector((state) => state.eventSlice);
    const dispatch = useDispatch()

    return (
        <>
            <button
                id="showActionFormBtn"
                style={{ display: "none" }}
                data-modal-target="actionForm"
                data-modal-toggle="actionForm"
            ...
            <div
                ...
                tabindex="-1"
                aria-hidden="true"
                class="fixed top-0 left-0 right-0 hidden z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full"
            >
                <div class="relative w-full h-full max-w-2xl md:h-auto">
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex justify-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                            ...
                            <button
                                onClick={() => dispatch(deleteEvent(selectedEvent.id))}
                                data-modal-hide="actionForm"
                                type="button"
